...
impl Gpu {
    ...
    fn kv_write_v_by_mode(
        ...
        match v_mode_bits {
            ...
            _ => self.kv_cache_write_q8_0(v_dst, v_src, pos_buf, n_kv_heads, head_dim, 0),
        }
    }
    ...
    fn launch_asym_flash_batched(
        ...
        block_cols: usize,
        v_mode_bits: i32,
        // Sliding-window lookback (Gemma 4). 0 = no window / full causal.
        window_size: u32,
        // Ring-buffer cache capacity (Gemma 4 sliding KV). 0 = no wrap.
        cache_capacity: u32,
    ) -> HipResult<()> {
        const TILE_SIZE: usize = 128;
        ...
        while offset < batch_size {
            let chunk = (batch_size - offset).min(sub_batch);
            {
                ...
                let vm = v_mode_bits;
                    let ws_param = window_size as i32;
                    let cc_param = cache_capacity as i32;
                let mut params: Vec<*mut c_void> = vec![
                    &q_ptr as *const _ as *mut c_void,
                    &k_ptr as *const _ as *mut c_void,
                    &v_ptr as *const _ as *mut c_void,
                    ...
                    &ws_param as *const _ as *mut c_void,
                    &cc_param as *const _ as *mut c_void,
                ];
                if !wmma_ok {
                ...
                self.launch_maybe_blob(
                    ...
                    || {
                        ...
                        b.push_i32(bs);
                        b.push_i32(bc);
                        b.push_i32(window_size as i32);
                        b.push_i32(cache_capacity as i32);
                        if !wmma_ok {
                            b.push_i32(vm);
                        }
                        b
    ...
    // ── Gemma 4 sliding-window attention (window_size + cache_capacity) ──
    // Ported from jukefr/gemma4-128k-ring-buffer dispatch.rs
    ...
    // ── Gemma 4 partial RoPE (rotate_half convention) ──
    // Ported from jukefr/gemma4-128k-ring-buffer dispatch.rs
