...
impl Gpu {
    ...
    // ═══ Graduated KV cache conversion ═══

    /// Convert Q8_0 KV cache → turbo4 format for positions 0..convert_len.
    pub fn kv_cache_convert_q8_to_turbo4(
        &mut self, q8_cache: &GpuTensor, turbo_cache: &GpuTensor,
        signs1: &GpuTensor, signs2: &GpuTensor,
        n_kv_heads: usize, head_dim: usize, convert_len: usize,
    ) -> HipResult<()> {
        self.ensure_kernel("kv_cache_convert_q8_to_turbo4", kernels::KV_CACHE_CONVERT_Q8_TO_TURBO4_SRC, "kv_cache_convert_q8_to_turbo4")?;
        let func = &self.functions["kv_cache_convert_q8_to_turbo4"];
        let mut q8p = q8_cache.buf.as_ptr();
        let mut tp = turbo_cache.buf.as_ptr();
        let mut s1p = signs1.buf.as_ptr();
        let mut s2p = signs2.buf.as_ptr();
        let mut nh = n_kv_heads as i32;
        let mut hd = head_dim as i32;
        let mut cl = convert_len as i32;
        let mut params: Vec<*mut c_void> = vec![
            &mut q8p as *mut _ as *mut c_void, &mut tp as *mut _ as *mut c_void,
            &mut s1p as *mut _ as *mut c_void, &mut s2p as *mut _ as *mut c_void,
            &mut nh as *mut _ as *mut c_void, &mut hd as *mut _ as *mut c_void,
            &mut cl as *mut _ as *mut c_void,
        ];
        let shared = (head_dim + 32) * 4; // x[head_dim] + scratch[32]
        unsafe { self.hip.launch_kernel(func, [n_kv_heads as u32, convert_len as u32, 1], [32, 1, 1], shared as u32, self.stream_ref(), &mut params) }
    }

    /// Convert turbo4 KV cache → turbo2 format for positions 0..convert_len.
    pub fn kv_cache_convert_turbo4_to_turbo2(
        &mut self, t4_cache: &GpuTensor, t2_cache: &GpuTensor,
        n_kv_heads: usize, head_dim: usize, convert_len: usize,
    ) -> HipResult<()> {
        self.ensure_kernel("kv_cache_convert_turbo4_to_turbo2", kernels::KV_CACHE_CONVERT_TURBO4_TO_TURBO2_SRC, "kv_cache_convert_turbo4_to_turbo2")?;
        let func = &self.functions["kv_cache_convert_turbo4_to_turbo2"];
        let mut t4p = t4_cache.buf.as_ptr();
        let mut t2p = t2_cache.buf.as_ptr();
        let mut nh = n_kv_heads as i32;
        let mut hd = head_dim as i32;
        let mut cl = convert_len as i32;
        let mut params: Vec<*mut c_void> = vec![
            &mut t4p as *mut _ as *mut c_void, &mut t2p as *mut _ as *mut c_void,
            &mut nh as *mut _ as *mut c_void, &mut hd as *mut _ as *mut c_void,
            &mut cl as *mut _ as *mut c_void,
        ];
        let shared = 32 * 4; // scratch[32]
        unsafe { self.hip.launch_kernel(func, [n_kv_heads as u32, convert_len as u32, 1], [32, 1, 1], shared as u32, self.stream_ref(), &mut params) }
    }
}
