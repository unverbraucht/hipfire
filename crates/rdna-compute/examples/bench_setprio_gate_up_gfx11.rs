...
// of gemm_gate_up_mq4g256_lloyd_wmma (generic, runs on all gfx11).
// Issue #344: FeatherOps s_setprio investigation, gfx1100 re-test.
//
// Run: cargo run --release --example bench_setprio_gate_up_gfx11 -p rdna-compute
...
fn run_variant(
    ...
    println!(
        "  {:40}  median={:8.1} µs  min={:8.1} µs  {:5.1} GiB/s",
        label, median_us, min_us, gbps
    );
    median_us
}
...
fn main() {
    ...
    println!("gfx1100 s_setprio microbenchmark — {} runs (median of), {} warmup", runs, warmup);
    println!();
    ...
    for &(m, k, n, desc) in &shapes {
        ...
        let spb = run_variant(
            "SETPRIO-B (around B-tile loads in K-loop)",
            "gemm_gate_up_setprioB_gfx11",
            SETPRIO_B_SRC,
            "gemm_gate_up_mq4g256_lloyd_wmma",
            &a_gate, &a_up, &x_u8, gm, um, k, n, warmup, runs,
        );

        let da = (orig - spa) / orig * 100.0;
        let db = (orig - spb) / orig * 100.0;
        println!("  >>> SETPRIO-A vs orig: {:+.1}%  ({:.1} vs {:.1} µs)", da, orig, spa);
        println!("  >>> SETPRIO-B vs orig: {:+.1}%  ({:.1} vs {:.1} µs)", db, orig, spb);
        println!();
    }
}
