...
enum QuantType {
    ...
    MG4G256 = 25,  // Magnum-Gemma 4-bit: same MQ4 binary (136 B/group), engine
...
fn main() {
    ...
    for (name, file_idx) in &all_tensors {
        ...
        if is_moe_expert_3d {
            ...
            let expert_promoted = matches!(kmap.get(*name).copied(), Some(QuantLevel::Promote6));
            let expert_mq6 = (use_mq6g256 || use_mq4_mq6exp) && supports_g256;
            let expert_hfq6 = use_hfq6 && supports_g256;
            let expert_hfq4 = use_hfq4g256 && !expert_promoted && supports_g256;
            ...
            let mut new_tensors: Vec<HfqTensor> = (0..n_experts).into_par_iter().map(|x| {
                ...
                let (quantized, qt, gs) = if expert_promoted && supports_g256 {
                    // Promoted (Promote6 → MQ6G256).
                    let q = quantize_mq6g256(&f32_slice, &signs1, &signs2);
                    (q, QuantType::MQ6G256, 256u32)
                } else if expert_promoted {
                    // Promoted but non-256-aligned: fall to HFQ4G128.
            ...
            let label = if expert_promoted {
                if supports_g256 { "MQ6G256" } else { "HFQ4G128" }
    ...
    // Take ownership of spill for cleanup (cleanup takes self)
    if let Some(mut s) = spill.take() { s.cleanup(); }
