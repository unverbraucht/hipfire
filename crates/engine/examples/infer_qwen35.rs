...
//! `engine::prompt_frame::ChatFrame`, top-p sampling via
//! `engine::sampler::sample`, output-stream filtering via
//! `engine::eos_filter::EosFilter`, and the n-gram repetition detector
//! via `engine::loop_guard::LoopGuard`. The default keeps today's bare
...
use engine::llama;
use engine::loop_guard::LoopGuard;
use engine::prompt_frame::{AssistantPrefix, ChatFrame};
use engine::qwen35;
...
fn main() {
    ...
    let prompt_tokens = if use_guards {
        // Production framing path: route through engine::prompt_frame so
