...
///
/// TODO(transformer-extraction): cross-arch duplicate. The Qwen2 variant
/// in `hipfire-arch-qwen2::qwen2::load_norm_weight_raw` is structurally
/// identical except it omits the `model.language_model.` name prefix
/// (Qwen2 stores norms flat under `model.{...}`). Pull into
/// `hipfire_runtime::transformer::norm` during consolidation.
