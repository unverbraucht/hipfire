...
**TODO — BF16 LA-layer confirmation test for the GLM-5 "quant noise" hypothesis (2026-05-12).**

Independent audit `deltanet_glm5.md` reviewed all 6 upstream stages feeding `gated_delta_net` plus the recurrence kernel line-by-line against hipfire / vLLM / llama.cpp, found all of them clean, and attributed the residual ~0.06 per-LA-layer to weight quantization noise amplified through the recurrence. Their methodology is solid (three-engine code review > my two-engine probe) and their RoPE finding is identical to ours. But their quant-noise attribution remains unverified:
  - GLM-5 reports the result as "Q4 quantization noise", citing the `fused_qkvza_hfq4g256` kernel — but the matched measurement vehicle (Q3.5-0.8B q8f16) uses Q8 weights via `gemv_q8_0`, not HFQ4. The per-layer dump showing layer-4 rel_L2 = 0.14 is on Q8 weights, so the bpw attribution doesn't carry over directly.
  - GLM-5's proposed confirmation test was not run: re-quantize Q3.5-0.8B with Q8 weights for everything EXCEPT the DeltaNet layers, which get BF16 (or FP16) storage, and re-dump. If layer-4 rel_L2 drops dramatically (say to ≤ 0.02), the LA-layer drift IS weight quantization noise amplified by the recurrence — a fixable-by-higher-precision floor. If it stays ~0.14, GLM-5's line-by-line review missed something upstream of DeltaNet (most likely a subtle convention/precision mismatch in one of: `conv1d_silu_split`, `fused_qk_l2_norm_scale`, or the sigmoid/alpha-gate transform).

Implementation cost: ~2-3 h. `hipfire-quantize` already has `HIPFIRE_QUANTIZE_LM_HEAD_F16` env-gate logic (commit df11148f) — generalize the pattern to a `HIPFIRE_QUANTIZE_LA_LAYERS_F16` (or similar) that forces F16/BF16 storage on `linear_attn.*` weights. Re-quantize the 0.8B, re-dump per-layer hidden states, re-compare. Then either:
