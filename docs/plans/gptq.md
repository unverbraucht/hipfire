...
3. **Sequencing AWQ before vs after GPTQ in the quantize chain.** Default: AWQ first, GPTQ on AWQ-scaled weights. The reverse order doesn't compose cleanly (GPTQ would optimize on un-AWQ-scaled weights; AWQ post-multiply would re-introduce noise).
4. **Hessian collection corpus.** Reuse wikitext-2-train via existing `imatrix_collect` integration vs commit to a separate calibration set. Default: reuse. The risk of overfitting to one dataset is real but small relative to the bug-surface area of a second calibration pipeline.
5. **Do we need to skip MoE experts initially?** Their per-expert input distribution is conditional on the router; collecting per-expert Hessians needs the routing decisions, which adds complexity. Default: skip MoE experts for v1 (Stage B is dense + linear_attn only). Add expert-Hessians in Stage B.1 if Stage B wins on dense.
