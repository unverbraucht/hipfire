...
| `kld-measurements-master.md` | **Master KLD/PPL catalog.** Consolidated table of every measurement against the BF16 reference, across model sizes, KV modes, and engines (hipfire / llama.cpp GGUF). Source-of-truth; other docs pull derived views. Methodology rules in §6. |
...
| `mr-gptq-calibration-investigation.md` | **MR-GPTQ vs imatrix for L5c.** Castro et al. (arXiv 2509.23202) introduce GPTQ + block-Hadamard-32 + MSE-optimized scale grid + static activation reordering — directly targets the FWHT-FP4 incompatibility fivetide exposed (+3.8pp recovery on MXFP4 vs vanilla GPTQ). Adds MR-GPTQ as third candidate in Phase A Step 5; gfx906 viability analysis shows block-Hadamard-32 should be *cheaper* than our current FWHT-256 online rotation. Fits the existing HFP4 wire-format reserved bits (no breaking change). |
