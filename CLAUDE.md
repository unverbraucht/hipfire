...
4. **Portability matters.** Every decision should consider: will this work on gfx906 (CDNA1 / Vega 20, MI50)? RDNA2? RDNA3? RDNA4? gfx906 is a supported production target (see `docs/perf-checkpoints/2026-05-{05,07,11}-gfx906-*` for the dp4a-based path); its wave64 + `__builtin_amdgcn_sdot4` + no-WMMA profile differs sharply from RDNA, so kernels added or modified should either explicitly cover gfx906 with a sister implementation (the HFQ4 / HFQ6 / HFP4 family does this) or document why it's deferred. If it's 5700XT-only — or gfx11+-only — it's a hack, not a solution.
